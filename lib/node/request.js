...
function _initHeaders(req) {
  var ua = 'node-superagent/'.concat(pkg.version);
  req._header = {
...
Request.prototype.accept = function(type) {
  return this.set('Accept', ~type.indexOf('/') ? type : mime.getType(type));
};
/**
...
Request.prototype._pipeContinue = function(stream, options) {
  ...
  this.req.once('response', function(res) {
    ...
    if (redirect && _this2._redirects++ != _this2._maxRedirects) {
      return _this2._redirect(res)._pipeContinue(stream, options);
    }
...
Request.prototype.request = function() {
  ...
  var req = (this.req = mod.request(options)); // set tcp no delay
